---
title: "Are the popular physical wearable devices fair?"
subtitle: "Investigating customer demographics and product performance for MINGAR wearable devices"
...
```{r, include = TRUE, echo = FALSE}
ggarrange(race_fig_2, ggarrange(race_fig_1, race_fig_3, labels = c("6", "7"), ncol = 1, nrow = 2), labels = c("5"))
```

Figure 5 shows that there are more light emoji modifier users than dark emoji modifier users and there are more females than males. Most users use the Advance or Run lines.

Figure 6 shows that darker emoji modifier users seem to have a larger spread and larger mean of number of flags than compared to lighter emoji modifier users, suggesting some evidence of a performance bias.

Figure 7 shows that there is not much relationship between duration of sleep and number of flags, indicating that duration may not be an important predictor for the number of flags.
...
```{r, include = TRUE, echo = FALSE, message=FALSE, fig.align='left', fig.width=4, fig.height = 3, fig.cap="Boxplot of variance of log(# of flags) for all users"}
...
```{r echo=FALSE, include=TRUE, fig.height=3, fig.width=3, fig.cap="Percentage of devices that have certain Functionalities for each Line"}
