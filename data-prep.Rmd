...
### "racist data"
```{r}
race_sleep_data <- customer %>% full_join(cust_sleep, by = "cust_id") %>% select("cust_id", "sex", "emoji_modifier", "date", "duration", "flags") %>% filter(!is.na(date)) %>% filter(!is.na(duration)) %>% filter(!is.na(flags)) %>% filter(!is.na(emoji_modifier))

race_sleep_data <- race_sleep_data %>% full_join(cust_dev, by = "cust_id") 

race_sleep_data <- race_sleep_data %>% full_join(device, by = "dev_id") %>% select("cust_id", "sex", "emoji_modifier", "date", "duration", "flags", "dev_id", "device_name")

device_data <- device_data %>% janitor::clean_names() 

race_sleep_data <- race_sleep_data %>% full_join(device_data, by = "device_name") %>% select("cust_id", "sex", "emoji_modifier", "date", "duration", "flags", "device_name", "line", "battery_life", "water_resitance", "heart_rate_sensor", "pulse_oximiter", "gps", "sleep_tracking", "smart_notifications")
